...
struct NmsConfig {
  ...
  float iou_threshold = 0.1f;             ///< IoU above which a lower-scored box is suppressed.
  int max_detections = 64;                ///< Maximum number of boxes retained.
  float internal_score_threshold = 0.5f;  ///< Score pivot used when rescaling class thresholds.
};
